...
def pathFile(FileName):
    """
    return path file from current dir (listOK.txt)
    """
    cat = os.getcwd()
...
def checkQuarter(data):
    ...
    a.to_csv('listOK.txt',sep=';',index=False, header=True)
    return print("listOK saved")
#checkQuarter(data())


def RofP(data1,dataSet):
    """
    Regularity of prduct
    """ 
    customer = data1['idCus'].drop_duplicates().tolist()#customer list
    ...
    for i in customer:
        ...
        df0 = data1[data1.idCus == i]
        idProduct = df0['sku'].drop_duplicates().tolist() #product list
